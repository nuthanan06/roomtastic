...
class Database:
    def __init__(self):
        ...
        if not url or not key:
            raise ValueError("Missing SUPABASE_URL or SUPABASE_KEY environment variables. Check your .env or environment.")
    ...
    def upload_image_and_get_link(self, file_path) -> str:
        ...
        """
        try:
            bucket_name = os.environ.get("SUPABASE_BUCKETNAME")  # Make sure this bucket exists
            if not bucket_name:
            ...
            try:
                upload_result = self.supabase.storage.from_(bucket_name).upload(dest, file_content)
                ...
                try:
                    ...
                    raise RuntimeError(f"Failed to upload to Supabase storage: {e2}") from e
            ...
            if isinstance(pub, dict):
                ...
                url = getattr(pub, "publicURL", None) or getattr(pub, "publicUrl", None) or (pub if isinstance(pub, str) else None)

            return url or ""
        except Exception as e:
            msg = str(e)
            if "Bucket not found" in msg or "bucket not found" in msg.lower():
                print("Error: bucket not found. Check SUPABASE_BUCKETNAME and that the bucket exists in your Supabase project.")
                print(f"Configured bucket: {bucket_name}")
                print("To fix: create the bucket in the Supabase dashboard (Storage → Buckets) or set SUPABASE_BUCKETNAME to an existing bucket.")
                print("If you want the file to be accessible publicly, make the bucket public or use signed URLs (create_signed_url).")
            else:
