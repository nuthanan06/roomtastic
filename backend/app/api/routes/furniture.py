...
def create_furniture(room_id: UUID, body: FurnitureCreate, db: Session = Depends(get_db)):
...
def patch_furniture(furniture_id: UUID, body: FurnitureUpdate, db: Session = Depends(get_db)):
...
def move_furniture_ep(furniture_id: UUID, body: FurnitureMoveBody, db: Session = Depends(get_db)):
    f = ctrl.move_furniture(db, furniture_id, body)
    return FurnitureOut.model_validate(f)


@router.patch("/furniture/{furniture_id}/rotate", response_model=FurnitureOut)
def rotate_furniture_ep(furniture_id: UUID, body: FurnitureRotateBody, db: Session = Depends(get_db)):
